...
// added end by yingmingbo
  ...
  annotationDeclaration(context) {
    ...
    if (context.value[0].name === 'variablesDeclaration') {
      ...
      if (variables.length === 1 && typeof variables[0] === 'string') {
        ...
        return { optionName: context.option[0].image, optionValue: variables, type: 'BINARY' };
      }
    }
  ...
  annotationDeclarationOfTo(context) {
    if (!context.value) {
      return { optionName: context.option[0].image, type: 'UNARY' };
    }
...
// added begin by yingmingbo
  ...
  variableDeclaration(context) {
    if (context.STRING) {
      var variable = {};
      variable[context.NAME[0].image] = context.STRING[0].image;
      return variable;
    } else {
      return context.NAME[0].image;
    }
...
// added end by yingmingbo
  ...
  relationshipBody(context) {
    ...
    if (context.annotationDeclarationOfTo) {
      context.annotationDeclarationOfTo.forEach(contextObject => {
        optionsOfTo.push(this.visit(contextObject));
      });
    }
    ...
    return { from, to, options, optionsOfTo };
  }
...
// modified begin by yingmingbo
      if (context.injectedFieldParams && context.injectedFieldParams[0].children) {
        injectedField += '(';
        let injectedFieldParams = [];
        context.injectedFieldParams[0].children.injectedFieldParam.forEach(element => {
          let fields = [];
          element.children.NAME.forEach(field => {
            fields.push(field.image);
          })
          injectedFieldParams.push(fields.join('.'));
        });
        injectedField += injectedFieldParams.join(',');
        injectedField += ')';
      }
...
// added begin by yingmingbo
  injectedFieldParams(context) {
    return context.injectedFieldParams.map(this.visit, this).reduce((final, current) => [...final, current], []);
  }
  injectedFieldParam(context) {
    return context.injectedFieldParam.map(this.visit, this).reduce((final, current) => [...final, current], []);
  }
// added end by yingmingbo
