...
// added begin by yingmingbo
  ...
  datasetDeclaration(context) {
    ...
    return {name: datasetName, set: trimDataset(datasetBody)};
  }
  ...
  datasetBody(context) {
    return context.DATASET_BODY[0].image;
  }
  ...
  jsonConfigDeclaration(context) {
    ...
    return {name: jsonConfigName, set: trimDataBlock(jsonConfigBody)};
  }
...
// modified begin by yingmingbo
      ...
      console.log(name + ' injectedField end:');
...
function trimDataBlock(dataset) {
  let ret = dataset.replace(/^\/\*\#/, '').replace(/[*]+\/$/, '');
  console.log(dataset);
  console.log(ret);
}
// added end by yingmingbo
