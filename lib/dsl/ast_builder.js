...
class JDLAstBuilderVisitor extends BaseJDLCSTVisitor {
  ...
  applicationSubDeclaration(context) {
    ...
    if (context.applicationSubEntityOptions) {
      applicationSubDeclaration.options = this.visit(context.applicationSubEntityOptions);
    }
  ...
  applicationSubEntityOptions(context) {
    const options = [];
    Object.keys(context).forEach(optionDeclarationName => {
      context[optionDeclarationName].forEach(optionDeclaration => {
        const name = optionContextToAstMapping[optionDeclarationName];
        const optionDetails = this.visit(optionDeclaration);
        optionDetails.value = optionDetails.name;
        delete optionDetails.name;
        options.push({ name, ...optionDetails });
      });
    });
    return options;
  }
