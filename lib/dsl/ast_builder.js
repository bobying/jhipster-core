...
// added begin by yingmingbo
    ...
    if (context.filesDeclaration) {
      ast.files = _.flatMap(context.filesDeclaration, item => this.visit(item));
    }
// added end by yingmingbo
  ...
  entityDeclaration(context) {
    ...
    let externs = [];
    if (context.entityExternDeclaration) {
      externs = this.visit(context.entityExternDeclaration);
    }
...
// added begin by yingmingbo
  ...
  variableDeclaration(context) {
    if (context.variableValueDeclaration) {
      return this.visit(context.variableValueDeclaration);
    } else if (context.variableNameWithValueDeclaration) {
      return this.visit(context.variableNameWithValueDeclaration);
    }
  }

  variableNameWithValueDeclaration(context) {
    const key = this.visit(context.variableNameDeclaration);
    const value = this.visit(context.variableValueDeclaration);
    return {
      key,
      value
    };
  ...
  variableNameDeclaration(context) {
    if (context.NAME) {
      return context.NAME[0].image;
    } else if (context.STRING) {
      const variableImage = context.STRING[0].image;
      return variableImage.substring(1, variableImage.length - 1);
    }
  }

  variableValueDeclaration(context) {
    if (context.NAME) {
      return context.NAME[0].image;
    } else if (context.STRING) {
      const variableImage = context.STRING[0].image;
      ...
      return context.DECIMAL[0].image;
    }
  }
  ...
  pageDeclaration(context) {
    if (context.pageUpdateDeclaration) {
      return this.visit(context.pageUpdateDeclaration)
    }

    if (context.pageListDeclaration) {
      return this.visit(context.pageListDeclaration)
    }

    if (context.pageDetailDeclaration) {
      return this.visit(context.pageDetailDeclaration)
    }

    if (context.pageReportDeclaration) {
      return this.visit(context.pageReportDeclaration)
    }
  }

  pageUpdateDeclaration(context) {
    const annotations = [];
    if (context.annotationDeclaration) {
      context.annotationDeclaration.forEach(contextObject => {
        annotations.push(this.visit(contextObject));
  ...
  pageListDeclaration(context) {
    ...
    if (context.pageFilterDeclaration) {
      filters = this.visit(context.pageFilterDeclaration);
    }

    let cols = undefined;
    if (context.pageTableDeclaration) {
      cols = this.visit(context.pageTableDeclaration);
    }
  ...
  filesDeclaration(context) {
    const appname = context.NAME[0].image;
    const files = [];
    if (context.fileDeclaration) {
      context.fileDeclaration.forEach(contextObject => {
        files.push(this.visit(contextObject));
      });
    }
    return {
      appname,
  ...
  fileDeclaration(context) {
    let ret = {
      from: context.STRING[0].image,
      to: context.STRING[1].image,
    };
    ret.from = ret.from.substring(1, ret.from.length - 1);
    ret.to = ret.to.substring(1, ret.to.length - 1);
    return ret;
  }

  entityExternDeclaration(context) {
    const externs = [];
    context.NAME.forEach(item => {
      externs.push(item.image);
    });
    return externs;
  }
// added end by yingmingbo
