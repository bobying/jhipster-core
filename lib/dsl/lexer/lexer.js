...
createTokenFromConfig({ name: 'EXTERN', pattern: 'extern' }); // entity扩展
createTokenFromConfig({ name: 'JSONCONFIG', pattern: 'json' }); // json配置，支持代码块

// for page
createTokenFromConfig({ name: 'PAGE', pattern: 'page' }); // 页面配置，支持list、update、detail、report类型

createTokenFromConfig({ name: 'PAGE_UPDATE', pattern: 'update' });
createTokenFromConfig({ name: 'PAGE_LIST', pattern: 'list' });
createTokenFromConfig({ name: 'PAGE_DETAIL', pattern: 'detail' });
createTokenFromConfig({ name: 'PAGE_REPORT', pattern: 'report' });

createTokenFromConfig({ name: 'TAB', pattern: 'tab' });
createTokenFromConfig({ name: 'GROUP', pattern: 'group' });
createTokenFromConfig({ name: 'GRID', pattern: 'grid' });
createTokenFromConfig({ name: 'FIELD', pattern: 'field' });
createTokenFromConfig({ name: 'OPERATES', pattern: 'operates' });
createTokenFromConfig({ name: 'FILTERS', pattern: 'flts' });
createTokenFromConfig({ name: 'TABLE_COLS', pattern: 'cols' });
createTokenFromConfig({ name: 'REF', pattern: 'ref' }); // 页面引用，detail引用到update

createTokenFromConfig({ name: 'FILES', pattern: 'files' });  // 自定义文件添加

// Dataset-related
/*
function getDataset(text, startOffset) {
  let endOffset = startOffset;
  let charCode1 = text.charCodeAt(endOffset);
  let charCode2 = text.charCodeAt(endOffset + 1);
  // '}'
  while (!isNaN(charCode1) && !(charCode1 === 125)) {
    endOffset ++;
    charCode1 = text.charCodeAt(endOffset);
    charCode2 = text.charCodeAt(endOffset + 1);
  }
...
*/
createTokenFromConfig({ name: 'DATASET', pattern: 'dataset' }); // entity默认数据集
/*
createTokenFromConfig({
  name: 'DATASET_BODY',
  pattern: { exec: getDataset },
  line_breaks: true,
  start_chars_hint: ["/#"],
});
*/
