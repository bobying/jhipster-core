/** Copyright 2013-2020 the original author or authors from the JHipster project.
 *
 * This file is part of the JHipster project, see http://www.jhipster.tech/
 * for more information.
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *      http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

const { CstParser } = require('chevrotain');
const LexerTokens = require('./lexer/lexer').tokens;

let instance;

module.exports = class JDLParser extends CstParser {
  static getParser() {
    if (instance) {
      return instance;
    }
    instance = new JDLParser();
    return instance;
  }

  constructor() {
    super(LexerTokens);
  }

  parse() {
    this.prog();
// added begin by yingmingbo
    this.variablesDeclaration();
    this.variableDeclaration();
    this.variableNameWithValueDeclaration();
    this.variableNameDeclaration();
    this.variableValueDeclaration();
    this.entityExternDeclaration();
// added end by yingmingbo
    this.constantDeclaration();
    this.entityDeclaration();
    this.annotationDeclaration();
    this.entityTableNameDeclaration();
    this.entityBody();
    this.fieldDeclaration();
    this.type();
    this.validation();
    this.minMaxValidation();
    this.pattern();
// added begin by yingmingbo
    this.readonly();
    this.datasetDeclaration();
    this.datasetName();
    this.datasetBody();

    this.filesDeclaration();
    this.fileDeclaration();

    this.pageDeclaration();
    this.pageUpdateDeclaration();
    this.pageListDeclaration();
    this.pageDetailDeclaration();
    this.pageReportDeclaration();

    this.pageTabsDeclaration();
    this.pageOthersDeclaration();
    this.pageOtherDeclaration();
    this.pageTabDeclaration();
    this.pageGroupDeclaration();
    this.pageGridDeclaration();
    this.pageLeafDeclaration();
    this.pageFieldDeclaration();
    this.pageOperatesDeclaration();
    this.pageOperateDeclaration();
    this.pageActionDeclaration();
    this.pageFilterDeclaration();
    this.pageTableDeclaration();

    this.jsonConfigDeclaration();
    this.jsonConfigName();
    this.jsonConfigSubType();
    this.jsonConfigType();
    this.jsonConfigBody();

// added end by yingmingbo
    this.relationDeclaration();
    this.relationshipType();
    this.relationshipBody();
    this.relationshipSide();
    this.relationshipOptions();
    this.relationshipOption();
// added begin by yingmingbo
    this.injectedFieldParams();
    this.injectedFieldParam();
// added end by yingmingbo
    this.enumDeclaration();
    this.enumPropList();
    this.enumProp();
    this.entityList();
    this.exclusion();
    this.unaryOptionDeclaration();
    this.binaryOptionDeclaration();
    this.filterDef();
    this.comment();
    this.deploymentDeclaration();
    this.deploymentConfigDeclaration();
    this.deploymentConfigValue();
    this.applicationDeclaration();
    this.applicationSubDeclaration();
    this.applicationSubConfig();
    this.applicationSubEntities();
    this.applicationConfigDeclaration();
    this.configValue();
    this.qualifiedName();
    this.list();

    // very important to call this after all the rules have been defined.
    // otherwise the parsers may not work correctly as it will lack information
    // derived during the self analysis phase.
    this.performSelfAnalysis();
  }

  prog() {
    this.RULE('prog', () => {
      this.MANY(() => {
        this.OR([
          { ALT: () => this.SUBRULE(this.entityDeclaration) },
// added begin by yingmingbo
          { ALT: () => this.SUBRULE(this.datasetDeclaration) },
          { ALT: () => this.SUBRULE(this.filesDeclaration) },
          { ALT: () => this.SUBRULE(this.pageDeclaration) },
          { ALT: () => this.SUBRULE(this.jsonConfigDeclaration) },
// added end by yingmingbo
          { ALT: () => this.SUBRULE(this.relationDeclaration) },
          { ALT: () => this.SUBRULE(this.enumDeclaration) },
          { ALT: () => this.CONSUME(LexerTokens.JAVADOC) },
          { ALT: () => this.SUBRULE(this.unaryOptionDeclaration) },
          { ALT: () => this.SUBRULE(this.binaryOptionDeclaration) },
          { ALT: () => this.SUBRULE(this.applicationDeclaration) },
          { ALT: () => this.SUBRULE(this.deploymentDeclaration) },
          // a constantDeclaration starts with a NAME, but any keyword is also a NAME
          // So to avoid conflicts with most of the above alternatives (which start with keywords)
          // this alternative must be last.
          {
            // - A Constant starts with a NAME
            // - NAME tokens are very common
            // That is why a more precise lookahead condition is used (The GATE)
            // To avoid confusing errors ("expecting EQUALS but found ...")
            GATE: () => this.LA(2).tokenType === LexerTokens.EQUALS,
            ALT: () => this.SUBRULE(this.constantDeclaration)
          }
        ]);
      });
    });
  }

  constantDeclaration() {
    this.RULE('constantDeclaration', () => {
      this.CONSUME(LexerTokens.NAME);
      this.CONSUME(LexerTokens.EQUALS);
      this.OR([{ ALT: () => this.CONSUME(LexerTokens.DECIMAL) }, { ALT: () => this.CONSUME(LexerTokens.INTEGER) }]);
    });
  }

  entityDeclaration() {
    this.RULE('entityDeclaration', () => {
      this.OPTION(() => {
        this.CONSUME(LexerTokens.JAVADOC);
      });

      this.MANY(() => {
        this.SUBRULE(this.annotationDeclaration);
      });

      this.CONSUME(LexerTokens.ENTITY);
      this.CONSUME(LexerTokens.NAME);

      this.OPTION1(() => {
        this.SUBRULE(this.entityTableNameDeclaration);
      });

      this.OPTION2(() => {
        this.SUBRULE(this.entityExternDeclaration);
      });

      this.OPTION3(() => {
        this.SUBRULE(this.entityBody);
      });
    });
  }

  annotationDeclaration() {
    this.RULE('annotationDeclaration', () => {
      this.CONSUME(LexerTokens.AT);
      this.CONSUME(LexerTokens.NAME, { LABEL: 'option' });
      this.OPTION(() => {
        this.CONSUME(LexerTokens.LPAREN);
        this.OR([
// added begin by yingmingbo
          { ALT: () => this.SUBRULE(this.variablesDeclaration, { LABEL: 'value' })  },
// added end by yingmingbo
//          { ALT: () => this.CONSUME3(LexerTokens.NAME, { LABEL: 'value' }) },
//          { ALT: () => this.CONSUME3(LexerTokens.INTEGER, { LABEL: 'value' }) },
//          { ALT: () => this.CONSUME3(LexerTokens.DECIMAL, { LABEL: 'value' }) }
        ]);
        this.CONSUME(LexerTokens.RPAREN);
      });
    });
  }
// added begin by yingmingbo

  variablesDeclaration() {
    this.RULE('variablesDeclaration', () => {
      this.MANY(() => {
        this.SUBRULE(this.variableDeclaration);
        this.OPTION(() => {
          this.CONSUME(LexerTokens.COMMA);
        });
      });
    });
  }

  variableDeclaration() {
    this.RULE('variableDeclaration', () => {
      this.OR([
        { ALT: () => this.SUBRULE(this.variableNameWithValueDeclaration) },
        { ALT: () => this.SUBRULE(this.variableValueDeclaration) },
      ]);
    });
  }

  variableNameWithValueDeclaration() {
    this.RULE('variableNameWithValueDeclaration', () => {
      this.SUBRULE(this.variableNameDeclaration);
      this.CONSUME(LexerTokens.EQUALS);
      this.SUBRULE(this.variableValueDeclaration);
    });
  }

  variableNameDeclaration() {
    this.RULE('variableNameDeclaration', () => {
      this.OR([
        { ALT: () => this.CONSUME1(LexerTokens.NAME) },
        { ALT: () => this.CONSUME2(LexerTokens.STRING) },
      ]);
    });
  }

  variableValueDeclaration() {
    this.RULE('variableValueDeclaration', () => {
      this.OR([
        { ALT: () => this.CONSUME(LexerTokens.NAME) },
        { ALT: () => this.CONSUME1(LexerTokens.STRING) },
        { ALT: () => this.CONSUME2(LexerTokens.INTEGER) },
        { ALT: () => this.CONSUME3(LexerTokens.DECIMAL) },
      ]);
    });
  }

  entityExternDeclaration() {
    this.RULE('entityExternDeclaration', () => {
        this.CONSUME(LexerTokens.EXTERN);
        this.AT_LEAST_ONE_SEP({
          SEP: LexerTokens.COMMA,
          DEF: () => {
            this.CONSUME(LexerTokens.NAME);
          }
        });
      });
  }
// added end by yingmingbo

  entityTableNameDeclaration() {
    this.RULE('entityTableNameDeclaration', () => {
      this.CONSUME(LexerTokens.LPAREN);
      this.CONSUME(LexerTokens.NAME);
      this.CONSUME(LexerTokens.RPAREN);
    });
  }

  entityBody() {
    this.RULE('entityBody', () => {
      this.CONSUME(LexerTokens.LCURLY);
      this.MANY(() => {
        this.SUBRULE(this.fieldDeclaration);
        this.OPTION(() => {
          this.CONSUME(LexerTokens.COMMA);
        });
      });
      this.CONSUME(LexerTokens.RCURLY);
    });
  }

  fieldDeclaration() {
    this.RULE('fieldDeclaration', () => {
      this.OPTION(() => {
        this.CONSUME(LexerTokens.JAVADOC);
      });

      this.MANY(() => {
        this.SUBRULE(this.annotationDeclaration);
      });

      this.CONSUME(LexerTokens.NAME);
      this.SUBRULE(this.type);
      this.MANY1(() => {
        this.SUBRULE(this.validation);
      });

      this.OPTION2({
        GATE: () => {
          const prevTok = this.LA(0);
          const nextTok = this.LA(1);
          // simulate "SPACE_WITHOUT_NEWLINE" of the PEG parsers
          return prevTok.startLine === nextTok.startLine;
        },
        DEF: () => {
          this.CONSUME2(LexerTokens.JAVADOC);
        }
      });
    });
  }

  type() {
    this.RULE('type', () => {
      this.CONSUME(LexerTokens.NAME);
    });
  }

  validation() {
    this.RULE('validation', () => {
      this.OR([
        { ALT: () => this.CONSUME(LexerTokens.REQUIRED) },
        { ALT: () => this.CONSUME(LexerTokens.UNIQUE) },
        { ALT: () => this.SUBRULE(this.minMaxValidation) },
// added begin by yingmingbo
        { ALT: () => this.SUBRULE(this.readonly) },
// added end by yingmingbo
        { ALT: () => this.SUBRULE(this.pattern) }
      ]);
    });
  }

  minMaxValidation() {
    this.RULE('minMaxValidation', () => {
      // Note that "MIN_MAX_KEYWORD" is an abstract token and could match 6 different concrete token types
      this.CONSUME(LexerTokens.MIN_MAX_KEYWORD);
      this.CONSUME(LexerTokens.LPAREN);
      this.OR([
        { ALT: () => this.CONSUME(LexerTokens.DECIMAL) },
        { ALT: () => this.CONSUME(LexerTokens.INTEGER) },
        { ALT: () => this.CONSUME(LexerTokens.NAME) }
      ]);
      this.CONSUME(LexerTokens.RPAREN);
    });
  }

  pattern() {
    this.RULE('pattern', () => {
      this.CONSUME(LexerTokens.PATTERN);
      this.CONSUME(LexerTokens.LPAREN);
      this.CONSUME(LexerTokens.REGEX);
      this.CONSUME(LexerTokens.RPAREN);
    });
  }
// added begin by yingmingbo

  readonly() {
    this.RULE('readonly', () => {
      this.CONSUME(LexerTokens.READONLY);
      this.CONSUME(LexerTokens.LPAREN);
      this.CONSUME(LexerTokens.REGEX);
      this.CONSUME(LexerTokens.RPAREN);
    });
  }

  datasetDeclaration() {
    this.RULE('datasetDeclaration', () => {
      this.CONSUME(LexerTokens.DATASET);
      this.SUBRULE(this.datasetName);
      this.CONSUME(LexerTokens.LCURLY);
      this.SUBRULE(this.datasetBody);
      this.CONSUME(LexerTokens.RCURLY);
    });
  }

  datasetName() {
    this.RULE('datasetName', () => {
      this.CONSUME(LexerTokens.NAME);
    });
  }

  datasetBody() {
    this.RULE('datasetBody', () => {
      this.CONSUME(LexerTokens.DATA_BLOCK);
    });
  }

  pageDeclaration() {
    // page update on PurchaseOrder {
    this.RULE('pageDeclaration', () => {
      this.CONSUME(LexerTokens.PAGE);
      this.OR([
        { ALT: () => this.SUBRULE(this.pageUpdateDeclaration) },
        { ALT: () => this.SUBRULE(this.pageListDeclaration) },
        { ALT: () => this.SUBRULE(this.pageDetailDeclaration) },
        { ALT: () => this.SUBRULE(this.pageReportDeclaration) },
      ]);
    });
  }

  pageUpdateDeclaration() {
    // page update on PurchaseOrder {
    this.RULE('pageUpdateDeclaration', () => {
      this.CONSUME(LexerTokens.PAGE_UPDATE);
      this.MANY(() => {
        this.SUBRULE(this.annotationDeclaration);
      });
      this.OPTION(() => {
        this.CONSUME1(LexerTokens.NAME, { LABEL: 'name' });
      });
      this.CONSUME(LexerTokens.TO);
      this.CONSUME2(LexerTokens.NAME, { LABEL: 'entity' });
      this.CONSUME(LexerTokens.LCURLY);
      this.OR([
        { ALT: () => this.SUBRULE1(this.pageTabsDeclaration) },
        { ALT: () => this.SUBRULE2(this.pageOthersDeclaration) },
      ]);
      this.CONSUME(LexerTokens.RCURLY);
    });
  }

  pageListDeclaration() {
    // page update on PurchaseOrder {
    this.RULE('pageListDeclaration', () => {
      this.CONSUME(LexerTokens.PAGE_LIST);
      this.MANY(() => {
        this.SUBRULE(this.annotationDeclaration);
      });
      this.OPTION(() => {
        this.CONSUME1(LexerTokens.NAME, { LABEL: 'name' });
      });
      this.CONSUME(LexerTokens.TO);
      this.CONSUME2(LexerTokens.NAME, { LABEL: 'entity' });
      this.CONSUME(LexerTokens.LCURLY);
      this.SUBRULE1(this.pageFilterDeclaration);
      this.SUBRULE2(this.pageTableDeclaration);
      this.CONSUME(LexerTokens.RCURLY);
    });
  }

  pageDetailDeclaration() {
    // page update on PurchaseOrder {
    this.RULE('pageDetailDeclaration', () => {
      this.CONSUME(LexerTokens.PAGE_DETAIL);
      this.MANY(() => {
        this.SUBRULE(this.annotationDeclaration);
      });
      this.OPTION(() => {
        this.CONSUME1(LexerTokens.NAME, { LABEL: 'name' });
      });
      this.OPTION1(() => {
        this.CONSUME(LexerTokens.REF);
        this.CONSUME2(LexerTokens.NAME, { LABEL: 'ref' });
      });
      this.CONSUME(LexerTokens.TO);
      this.CONSUME(LexerTokens.NAME, { LABEL: 'entity' });
      this.OPTION2(() => {
        this.CONSUME(LexerTokens.LCURLY);
        this.OR([
          { ALT: () => this.SUBRULE1(this.pageTabsDeclaration) },
          { ALT: () => this.SUBRULE2(this.pageOthersDeclaration) },
        ]);
        this.CONSUME(LexerTokens.RCURLY);
      });
    });
  }

  pageReportDeclaration() {
    // page report on PurchaseOrder {
    this.RULE('pageReportDeclaration', () => {
      this.CONSUME(LexerTokens.PAGE_REPORT);
      this.MANY(() => {
        this.SUBRULE(this.annotationDeclaration);
      });
      this.OPTION(() => {
        this.CONSUME1(LexerTokens.NAME, { LABEL: 'name' });
      });
      this.CONSUME(LexerTokens.TO);
      this.CONSUME2(LexerTokens.NAME, { LABEL: 'entity' });
      this.CONSUME(LexerTokens.LCURLY);
      this.SUBRULE1(this.pageFilterDeclaration);
      this.SUBRULE2(this.pageTableDeclaration);
      this.CONSUME(LexerTokens.RCURLY);
    });
  }

  pageTabsDeclaration() {
    this.RULE('pageTabsDeclaration', () => {
      this.AT_LEAST_ONE(() => {
        this.SUBRULE(this.pageTabDeclaration);
      });
    });
  }

  pageOthersDeclaration() {
    this.RULE('pageOthersDeclaration', () => {
      this.AT_LEAST_ONE(() => {
        this.SUBRULE(this.pageOtherDeclaration);
      });
    });
  }

  pageOtherDeclaration() {
    this.RULE('pageOtherDeclaration', () => {
      this.OR([
        { ALT: () => this.SUBRULE1(this.pageGroupDeclaration) },
        { ALT: () => this.SUBRULE2(this.pageGridDeclaration) },
        { ALT: () => this.SUBRULE3(this.pageFieldDeclaration) }
      ]);
    });
  }

  pageTabDeclaration() {
    this.RULE('pageTabDeclaration', () => {
      this.CONSUME(LexerTokens.TAB);
      this.MANY1(() => {
        this.SUBRULE(this.annotationDeclaration);
      });
      this.CONSUME(LexerTokens.NAME);
      this.CONSUME(LexerTokens.LCURLY);
      this.SUBRULE(this.pageOthersDeclaration);
      this.CONSUME(LexerTokens.RCURLY);
    });
  }

  pageGroupDeclaration() {
    this.RULE('pageGroupDeclaration', () => {
      this.CONSUME(LexerTokens.GROUP);
      this.MANY1(() => {
        this.SUBRULE(this.annotationDeclaration);
      });
      this.CONSUME(LexerTokens.NAME);
      this.CONSUME(LexerTokens.LCURLY);
      this.SUBRULE(this.pageOthersDeclaration);
      this.CONSUME(LexerTokens.RCURLY);
    });
  }

  pageGridDeclaration() {
    this.RULE('pageGridDeclaration', () => {
      this.CONSUME(LexerTokens.GRID);
      this.MANY1(() => {
        this.SUBRULE(this.annotationDeclaration);
      });
      this.CONSUME(LexerTokens.NAME);
      this.CONSUME(LexerTokens.LCURLY);
      this.MANY2(() => {
        this.SUBRULE(this.pageLeafDeclaration);
      });
      this.CONSUME(LexerTokens.RCURLY);
    });
  }

  pageFieldDeclaration() {
    this.RULE('pageFieldDeclaration', () => {
      this.CONSUME(LexerTokens.FIELD);
      this.MANY(() => {
        this.SUBRULE(this.annotationDeclaration);
      });
      this.MANY_SEP({
        SEP: LexerTokens.DOT,
        DEF: () => {
          this.CONSUME(LexerTokens.NAME);
        }
      });
    });
  }

  pageLeafDeclaration() {
    this.RULE('pageLeafDeclaration', () => {
      this.OR([
        { ALT: () => this.SUBRULE1(this.pageOperatesDeclaration) },
        { ALT: () => this.SUBRULE2(this.pageFieldDeclaration) }
      ]);
    });
  }

  pageOperatesDeclaration() {
    this.RULE('pageOperatesDeclaration', () => {
      this.CONSUME(LexerTokens.OPERATES);
      this.MANY1(() => {
        this.SUBRULE(this.annotationDeclaration);
      });
      this.CONSUME(LexerTokens.NAME);
      this.OPTION(() => {
        this.CONSUME(LexerTokens.LCURLY);
        this.MANY2(() => {
          this.SUBRULE(this.pageOperateDeclaration);
        });
        this.CONSUME(LexerTokens.RCURLY);
      });
    });
  }

  pageOperateDeclaration() {
    this.RULE('pageOperateDeclaration', () => {
      this.CONSUME(LexerTokens.NAME);
      this.MANY1(() => {
        this.SUBRULE(this.annotationDeclaration);
      });
      this.OPTION(() => {
        this.CONSUME(LexerTokens.LCURLY);
        this.MANY2(() => {
          this.SUBRULE(this.pageActionDeclaration);
        });
        this.CONSUME(LexerTokens.RCURLY);
      });
    });
  }

  pageActionDeclaration() {
    this.RULE('pageActionDeclaration', () => {
      this.CONSUME1(LexerTokens.NAME);
      this.OR([
        { ALT: () => this.CONSUME(LexerTokens.DATA_BLOCK) },
        { ALT: () => this.CONSUME(LexerTokens.STRING) },
        { ALT: () => this.CONSUME(LexerTokens.NAME) }
      ]);
    });
  }

  pageFilterDeclaration() {
    this.RULE('pageFilterDeclaration', () => {
      this.CONSUME(LexerTokens.FILTERS);
      this.MANY1(() => {
        this.SUBRULE(this.annotationDeclaration);
      });
      this.CONSUME(LexerTokens.LCURLY);
      this.MANY2(() => {
        this.SUBRULE(this.pageLeafDeclaration);
      });
      this.CONSUME(LexerTokens.RCURLY);
    });
  }

  pageTableDeclaration() {
    this.RULE('pageTableDeclaration', () => {
      this.CONSUME(LexerTokens.TABLE_COLS);
      this.MANY1(() => {
        this.SUBRULE(this.annotationDeclaration);
      });
      this.CONSUME(LexerTokens.LCURLY);
      this.MANY2(() => {
        this.SUBRULE(this.pageLeafDeclaration);
      });
      this.CONSUME(LexerTokens.RCURLY);
    });
  }

  jsonConfigDeclaration() {
    this.RULE('jsonConfigDeclaration', () => {
      this.CONSUME(LexerTokens.JSONCONFIG);
      this.SUBRULE(this.jsonConfigName);
      this.SUBRULE(this.jsonConfigType);
      this.SUBRULE(this.jsonConfigSubType);
      this.CONSUME(LexerTokens.LCURLY);
      this.SUBRULE(this.jsonConfigBody);
      this.CONSUME(LexerTokens.RCURLY);
    });
  }

  jsonConfigName() {
    this.RULE('jsonConfigName', () => {
      this.CONSUME(LexerTokens.NAME);
    });
  }

  jsonConfigType() {
    this.RULE('jsonConfigType', () => {
      this.CONSUME(LexerTokens.NAME);
    });
  }

  jsonConfigSubType() {
    this.RULE('jsonConfigSubType', () => {
      this.CONSUME(LexerTokens.NAME);
    });
  }

  jsonConfigBody() {
    this.RULE('jsonConfigBody', () => {
      this.CONSUME(LexerTokens.DATA_BLOCK);
    });
  }

  filesDeclaration() {
    this.RULE('filesDeclaration', () => {
      this.CONSUME(LexerTokens.FILES);
      this.CONSUME(LexerTokens.NAME);
      this.CONSUME(LexerTokens.LCURLY);
      this.MANY(() => {
        this.SUBRULE(this.fileDeclaration);
      });
      this.CONSUME(LexerTokens.RCURLY);
    });
  }

  fileDeclaration() {
    this.RULE('fileDeclaration', () => {
      this.CONSUME1(LexerTokens.STRING);
      this.CONSUME(LexerTokens.TO);
      this.CONSUME2(LexerTokens.STRING);
    });
  }
// added end by yingmingbo

  relationDeclaration() {
    this.RULE('relationDeclaration', () => {
      this.CONSUME(LexerTokens.RELATIONSHIP);
      this.SUBRULE(this.relationshipType);
      this.CONSUME(LexerTokens.LCURLY);
      this.AT_LEAST_ONE(() => {
        this.SUBRULE(this.relationshipBody);
        this.OPTION(() => {
          this.CONSUME(LexerTokens.COMMA);
        });
      });
      this.CONSUME(LexerTokens.RCURLY);
    });
  }

  relationshipType() {
    this.RULE('relationshipType', () => {
      this.CONSUME(LexerTokens.RELATIONSHIP_TYPE);
    });
  }

  relationshipBody() {
    this.RULE('relationshipBody', () => {
      this.MANY(() => {
        this.SUBRULE(this.annotationDeclaration);
      });
      this.SUBRULE1(this.relationshipSide, { LABEL: 'from' });
      this.CONSUME(LexerTokens.TO);
      this.SUBRULE2(this.relationshipSide, { LABEL: 'to' });
      this.OPTION(() => {
        this.CONSUME(LexerTokens.WITH);
        this.SUBRULE3(this.relationshipOptions, { LABEL: 'relationshipOptions' });
      });
    });
  }

  relationshipSide() {
    this.RULE('relationshipSide', () => {
      this.SUBRULE(this.comment);
// added begin by yingmingbo
      this.MANY1(() => {
        this.SUBRULE1(this.annotationDeclaration, { LABEL: 'annotations' });
      });
// added end by yingmingbo
      this.CONSUME(LexerTokens.NAME);
      this.OPTION(() => {
        this.CONSUME(LexerTokens.LCURLY);
// added begin by yingmingbo
        this.MANY2(() => {
          this.SUBRULE2(this.annotationDeclaration, { LABEL: 'annotations' });
        });
// added end by yingmingbo
        this.CONSUME2(LexerTokens.NAME, { LABEL: 'injectedField' });

        this.OPTION1(() => {
// modified begin by yingmingbo
          this.SUBRULE(this.injectedFieldParams);
// modified end by yingmingbo
        });

        this.OPTION2(() => {
// modified begin by yingmingbo
          this.OR([
            { ALT: () => this.CONSUME(LexerTokens.REQUIRED) },
            { ALT: () => this.CONSUME(LexerTokens.UNIQUE) },
            { ALT: () => this.SUBRULE(this.pattern) },
            { ALT: () => this.SUBRULE(this.readonly) }
          ]);
// modified end by yingmingbo
        });
        this.CONSUME(LexerTokens.RCURLY);
      });
    });
  }

  relationshipOptions() {
    this.RULE('relationshipOptions', () => {
      this.AT_LEAST_ONE_SEP({
        SEP: LexerTokens.COMMA_WITHOUT_NEWLINE,
        DEF: () => {
          this.SUBRULE(this.relationshipOption, { LABEL: 'relationshipOption' });
        }
      });
    });
  }
// added begin by yingmingbo

  injectedFieldParams() {
    this.RULE('injectedFieldParams', () => {
      this.CONSUME(LexerTokens.LPAREN);
      this.MANY_SEP({
        SEP: LexerTokens.COMMA,
        DEF: () => {
          this.SUBRULE(this.injectedFieldParam);
        }
      });
      this.CONSUME(LexerTokens.RPAREN);
    });
  }

  injectedFieldParam() {
    this.RULE('injectedFieldParam', () => {
      this.MANY(() => {
        this.SUBRULE(this.annotationDeclaration, { LABEL: 'annotations' } );
      });
      this.MANY_SEP({
        SEP: LexerTokens.DOT,
        DEF: () => {
          this.CONSUME(LexerTokens.NAME);
        }
      });
    });
  }
// added end by yingmingbo

  relationshipOption() {
    this.RULE('relationshipOption', () => {
      this.OR([{ ALT: () => this.CONSUME(LexerTokens.JPA_DERIVED_IDENTIFIER) }]);
    });
  }

  enumDeclaration() {
    this.RULE('enumDeclaration', () => {
      this.CONSUME(LexerTokens.ENUM);
      this.CONSUME(LexerTokens.NAME);
      this.CONSUME(LexerTokens.LCURLY);
      this.SUBRULE(this.enumPropList);
      this.CONSUME(LexerTokens.RCURLY);
    });
  }

  enumPropList() {
    this.RULE('enumPropList', () => {
      this.SUBRULE(this.enumProp);
      this.MANY(() => {
        this.CONSUME(LexerTokens.COMMA);
        this.SUBRULE1(this.enumProp);
      });
    });
  }

  enumProp() {
    this.RULE('enumProp', () => {
      this.CONSUME(LexerTokens.NAME, { LABEL: 'enumPropKey' });
      this.OPTION(() => {
        this.CONSUME(LexerTokens.LPAREN);
        this.CONSUME2(LexerTokens.NAME, { LABEL: 'enumPropValue' });
        this.CONSUME(LexerTokens.RPAREN);
      });
    });
  }

  entityList() {
    this.RULE('entityList', () => {
      this.commonEntityList();
      this.CONSUME(LexerTokens.WITH);
      this.OR1([
        { ALT: () => this.CONSUME2(LexerTokens.NAME, { LABEL: 'method' }) },
        { ALT: () => this.CONSUME3(LexerTokens.STRING, { LABEL: 'methodPath' }) }
      ]);
    });
  }

  commonEntityList() {
    this.MANY({
      // the next section may contain [NAME, WITH], LA(2) check is used to resolve this.
      GATE: () => this.LA(2).tokenType === LexerTokens.COMMA,
      DEF: () => {
        this.CONSUME(LexerTokens.NAME);
        this.CONSUME(LexerTokens.COMMA);
      }
    });
    this.OR([{ ALT: () => this.CONSUME(LexerTokens.STAR) }, { ALT: () => this.CONSUME1(LexerTokens.NAME) }]);
  }

  exclusion() {
    this.RULE('exclusion', () => {
      this.CONSUME(LexerTokens.EXCEPT);
      this.CONSUME(LexerTokens.NAME);
      this.MANY(() => {
        this.CONSUME(LexerTokens.COMMA);
        this.CONSUME2(LexerTokens.NAME);
      });
    });
  }

  unaryOptionDeclaration() {
    this.RULE('unaryOptionDeclaration', () => {
      this.CONSUME(LexerTokens.UNARY_OPTION);
      this.SUBRULE(this.filterDef);
      this.OPTION(() => {
        this.SUBRULE(this.exclusion);
      });
    });
  }

  binaryOptionDeclaration() {
    this.RULE('binaryOptionDeclaration', () => {
      this.CONSUME(LexerTokens.BINARY_OPTION);
      this.SUBRULE(this.entityList);
      this.OPTION(() => {
        this.SUBRULE(this.exclusion);
      });
    });
  }

  filterDef() {
    this.RULE('filterDef', this.commonEntityList);
  }

  comment() {
    this.RULE('comment', () => {
      this.OPTION(() => {
        this.CONSUME(LexerTokens.JAVADOC);
      });
    });
  }

  deploymentDeclaration() {
    this.RULE('deploymentDeclaration', () => {
      this.CONSUME(LexerTokens.DEPLOYMENT);
      this.CONSUME(LexerTokens.LCURLY);
      this.MANY(() => {
        this.OR([
          { ALT: () => this.CONSUME(LexerTokens.JAVADOC) },
          { ALT: () => this.SUBRULE(this.deploymentConfigDeclaration) }
        ]);
      });
      this.CONSUME(LexerTokens.RCURLY);
    });
  }

  deploymentConfigDeclaration() {
    this.RULE('deploymentConfigDeclaration', () => {
      this.CONSUME(LexerTokens.DEPLOYMENT_KEY);
      this.SUBRULE(this.deploymentConfigValue);
      this.OPTION(() => {
        this.CONSUME(LexerTokens.COMMA);
      });
    });
  }

  deploymentConfigValue() {
    this.RULE('deploymentConfigValue', () => {
      this.OR([
        { ALT: () => this.CONSUME(LexerTokens.BOOLEAN) },
        { ALT: () => this.SUBRULE(this.qualifiedName) },
        { ALT: () => this.SUBRULE(this.list) },
        { ALT: () => this.CONSUME(LexerTokens.INTEGER) },
        { ALT: () => this.CONSUME(LexerTokens.STRING) }
      ]);
    });
  }

  applicationDeclaration() {
    this.RULE('applicationDeclaration', () => {
      this.CONSUME(LexerTokens.APPLICATION);
      this.CONSUME(LexerTokens.LCURLY);
      this.SUBRULE(this.applicationSubDeclaration);
      this.CONSUME(LexerTokens.RCURLY);
    });
  }

  applicationSubDeclaration() {
    this.RULE('applicationSubDeclaration', () => {
      this.MANY(() => {
        this.OR([
          { ALT: () => this.SUBRULE(this.applicationSubConfig) },
          { ALT: () => this.SUBRULE(this.applicationSubEntities) }
        ]);
      });
    });
  }

  applicationSubConfig() {
    this.RULE('applicationSubConfig', () => {
      this.CONSUME(LexerTokens.CONFIG);
      this.CONSUME(LexerTokens.LCURLY);
      this.MANY(() => {
        this.OR([
          { ALT: () => this.CONSUME(LexerTokens.JAVADOC) },
          { ALT: () => this.SUBRULE(this.applicationConfigDeclaration) }
        ]);
      });
      this.CONSUME(LexerTokens.RCURLY);
    });
  }

  applicationConfigDeclaration() {
    this.RULE('applicationConfigDeclaration', () => {
      this.CONSUME(LexerTokens.CONFIG_KEY);
      this.SUBRULE(this.configValue);
      this.OPTION(() => {
        this.CONSUME(LexerTokens.COMMA);
      });
    });
  }

  configValue() {
    this.RULE('configValue', () => {
      this.OR([
        { ALT: () => this.CONSUME(LexerTokens.BOOLEAN) },
        { ALT: () => this.SUBRULE(this.qualifiedName) },
        { ALT: () => this.SUBRULE(this.list) },
        { ALT: () => this.CONSUME(LexerTokens.INTEGER) },
        { ALT: () => this.CONSUME(LexerTokens.STRING) }
      ]);
    });
  }

  qualifiedName() {
    this.RULE('qualifiedName', () => {
      this.AT_LEAST_ONE_SEP({
        SEP: LexerTokens.DOT,
        DEF: () => {
          this.CONSUME(LexerTokens.NAME);
        }
      });
    });
  }

  list() {
    this.RULE('list', () => {
      this.CONSUME(LexerTokens.LSQUARE);
      this.MANY_SEP({
        SEP: LexerTokens.COMMA,
        DEF: () => {
          this.CONSUME(LexerTokens.NAME);
        }
      });
      this.CONSUME(LexerTokens.RSQUARE);
    });
  }

  applicationSubEntities() {
    this.RULE('applicationSubEntities', () => {
      this.CONSUME(LexerTokens.ENTITIES);
      this.SUBRULE(this.filterDef);
      this.OPTION(() => {
        this.SUBRULE(this.exclusion);
      });
    });
  }
};
