...
// added begin by yingmingbo
    } else if (option.getType() === 'MAP') {
      addMapOption(this.options, option);
...
function addMapOption(options, optionToAdd) {
  const { name, value, entityNames } = optionToAdd;

  const entities = Array.from(entityNames.values());
  console.log(entities);
  if (entities.length !== 1) {
    const msg = 'Option: ' + name + ' has more than one entity: ';
    console.trace(entityNames);
    throw Error(msg);
  }
  const entityName = entities[0];
  if (!options[name]) {
    options[name] = {
      [entityName]: optionToAdd
    };
  } else if (!options[name][entityName]) {
    options[name][entityName] = optionToAdd;
  } else {
    options[name][entityName].addEntitiesFromAnotherOption(optionToAdd);
  }
}
// added end by yingmingbo
