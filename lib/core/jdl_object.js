...
class JDLObject {
  ...
  addRelationship(relationship, skippedUserManagementOption) {
    if (!relationship) {
      throw new Error("Can't add nil relationship.");
    }
    this.relationships.add(relationship, skippedUserManagementOption);
  }
  ...
  getOptionForName(optionName) {
    return this.applications[Object.keys(this.applications)[0]].config[optionName];
  }
