...
function fillApplicationOptions() {
  configuration.document.applications.forEach(application => {
    application.options.forEach(({ name, value, list, excluded }) => {
      if (UnaryOptions.exists(name)) {
        jdlObject.addOption(
          new JDLUnaryOption({
            name,
            entityNames: list,
...
function fillEntitiesAndFields() {
