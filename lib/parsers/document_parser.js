...
const JDLMapOption = require('../core/jdl_map_option');
...
function addFieldsOfExtern() {
  for (let i = 0, entityNames = jdlObject.getEntityNames(); i < entityNames.length; i++) {
    ...
    if (jdlEntity.externs && !jdlEntity.externsConverted) {
      console.trace(entityName);
      console.log(jdlEntity);
      convertExtern(jdlEntity);
      console.log('after');
      console.log(jdlEntity);
    }
  }
...
function convertExtern(jdlEntity) {
  if (jdlEntity.externs && !jdlEntity.externsConverted) {
    jdlEntity.externs.forEach(item => {
      let jdlEntityOfExtern = jdlObject.getEntity(item);
      if (jdlEntityOfExtern.externs && !jdlEntity.externsConverted) {
        convertExtern(jdlEntityOfExtern);
      }
      jdlEntity.fields = {...jdlEntityOfExtern.fields, ...jdlEntity.fields}; // TODO: how to deal duplicate fields
    });
    jdlEntity.externsConverted = true;
  }
}
...
// added begin by yingmingbo
      } else if (annotation.type === 'MAP') {
        jdlObject.entities[entityName][annotation.optionName] = annotation.optionValue;
        jdlObject.addOption(
          new JDLMapOption({
            name: annotation.optionName,
...
function fillDatasets() {
  parsedContent.datasets.forEach(dataset => {
    jdlObject.entities[dataset.name].dataset = dataset.set.trim();
  });
}
...
function fillFiles() {
  ...
  parsedContent.files.forEach(files => {
    parsedContent.applications.forEach(app => {
      if (app.config.baseName === files.appname) {
        jdlObject.applications[files.appname].files = files.files;
      }
    });
  });
}
...
function fillPages() {
  parsedContent.pages.forEach(page => {
    if (jdlObject.entities[page.entity]) {
      if (!jdlObject.entities[page.entity].page) {
        jdlObject.entities[page.entity].page = {};
      }
      if (!jdlObject.entities[page.entity].page[page.name]) {
        jdlObject.entities[page.entity].page[page.name] = {};
      }
      const convertedPage = convertPage(page, convertAnnotationsToOptions);

      jdlObject.entities[page.entity].page[page.name] = convertedPage;
    }
  });
}

function fillJsonConfigs() {
  parsedContent.jsonConfigs.forEach(jsonConfig => {
    if (jdlObject.entities[jsonConfig.name]) {
      if (!jdlObject.entities[jsonConfig.name].jsonConfig) {
        jdlObject.entities[jsonConfig.name].jsonConfig = {};
      }
      if (!jdlObject.entities[jsonConfig.name].jsonConfig[jsonConfig.set.type]) {
        jdlObject.entities[jsonConfig.name].jsonConfig[jsonConfig.set.type] = {};
      }
      jdlObject.entities[jsonConfig.name].jsonConfig[jsonConfig.set.type][jsonConfig.set.subtype] = jsonConfig.set;
    }
  });
}
// added end by yingmingbo
...
function convertAnnotationsToOptions(annotations) {
  ...
  annotations.forEach(annotation => {
    ...
    if (annotation.optionName in result) {
      ...
      if (annotation.key) {
        if (!result[annotation.key]) {
          result[annotation.key] = {}
        }
        result[annotation.key][annotation.optionName] = value;
      } else {
...
function convertPage(page, annotationToOptionConverter) {
  if (!page) {
    throw new Error('PageInfo have to be passed so as to be converted.');
  }

  page.options = annotationToOptionConverter.call(undefined, page.annotations);
  ...
  switch(page.type) {
    ...
    case 'action':
        break;
    default:
      console.trace('unknow type');
      console.log(page);
      break;
  }
  for (idf in page.values) {
    convertPage(page.values[idf], annotationToOptionConverter);
  }
  for (idf in page.fields) {
    convertPage(page.fields[idf], annotationToOptionConverter);
  }
  for (idf in page.operates) {
    convertPage(page.operates[idf], annotationToOptionConverter);
  }
  if (page.cols) {
    convertPage(page.cols, annotationToOptionConverter);
  }
  if (page.filters) {
    convertPage(page.filters, annotationToOptionConverter);
  }
  for (idf in page.actions) {
    convertPage(page.actions[idf], annotationToOptionConverter);
    page[page.actions[idf].key] = page.actions[idf].value;
  }
  return page;
}
// added end by yingmingbo
