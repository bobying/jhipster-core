...
function fillAssociations() {
  ...
  let skippedUserManagementOption = jdlObject.getOptionForName(ApplicationOptions.names.SKIP_USER_MANAGEMENT);
  jdlObject.relationshipValidator.skippedUserManagementOption = skippedUserManagementOption;
  jdlRelationships.forEach(jdlRelationship => {
    jdlObject.addRelationship(jdlRelationship, skippedUserManagementOption);
  });
}
