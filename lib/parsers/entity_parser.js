...
const { READONLY } = require('../core/jhipster/validations');
...
// added begin by yingmingbo
                && entityName.toLowerCase() !== AUTHORITY.toLowerCase()
// added end by yingmingbo
          )
      );
    }
...
// modified begin by yingmingbo
    if (entityName.toLowerCase() !== USER.toLowerCase() && entityName.toLowerCase() !== AUTHORITY.toLowerCase()) {
// modified end by yingmingbo
      setFieldsOfEntity(entityName);
      setRelationshipOfEntity(entityName);
// added begin by yingmingbo
      setDatasetOfEntity(entityName);
      setPageOfEntity(entityName);
      setJsonConfigOfEntity(entityName);
// added end by yingmingbo
    }
  }
}
...
function setDatasetOfEntity(entityName) {
  entities[entityName].dataset = jdlObject.entities[entityName].dataset;
}

function setPageOfEntity(entityName) {
  entities[entityName].page = jdlObject.entities[entityName].page;
}

function setJsonConfigOfEntity(entityName) {
  entities[entityName].jsonConfig = jdlObject.entities[entityName].jsonConfig;
}
// added end by yingmingbo
...
// modified begin by yingmingbo
      relationship.relationshipValidateRules.push(REQUIRED);
// modified end by yingmingbo
    }
// added begin by yingmingbo
    if (relatedRelationship.isInjectedFieldInFromReadonly) {
      relationship.relationshipValidateRules.push(READONLY);
      relationship.relationshipValidateRulesReadonly = relatedRelationship.isInjectedFieldInFromReadonly;
    }
// added end by yingmingbo
...
// modified begin by yingmingbo
      relationship.otherEntityField = (splitField.otherEntityField);
// modified end by yingmingbo
...
// modified begin by yingmingbo
          otherEntityField:(otherSplitField.otherEntityField),
          ownerSide: false,
// modified end by yingmingbo
        };
...
// added begin by yingmingbo for options on other side of relation
  relationship.options = relatedRelationship.options;
  if (relationship.ownerSide !== true) {
    relationship.optionsOfTo = relatedRelationship.optionsOfFrom;
    relationship.optionsOfFrom = relatedRelationship.optionsOfTo;
    relationship.optionsOfInjectedFieldInTo = relatedRelationship.optionsOfInjectedFieldInFrom;
    relationship.optionsOfInjectedFieldInFrom = relatedRelationship.optionsOfInjectedFieldInTo;
  } else {
    relationship.optionsOfTo = relatedRelationship.optionsOfTo;
    relationship.optionsOfFrom = relatedRelationship.optionsOfFrom;
    relationship.optionsOfInjectedFieldInTo = relatedRelationship.optionsOfInjectedFieldInTo;
    relationship.optionsOfInjectedFieldInFrom = relatedRelationship.optionsOfInjectedFieldInFrom;
  }
// added end by yingmingbo for options on other side of relation
}
...
// modified end by yingmingbo
    ...
    if (chunks.length > 1) {
// modified begin by yingmingbo
      splitField.otherEntityField = [];
      for (i = 1; i < chunks.length; i ++) {
        splitField.otherEntityField.push(chunks[i]);
      }
// modified end by yingmingbo
    }
  }
